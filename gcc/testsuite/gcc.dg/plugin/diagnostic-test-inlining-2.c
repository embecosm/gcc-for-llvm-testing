...
/* { dg-regexp "    inlined from 'bar' at .+/diagnostic-test-inlining-2.c:20:3," "" } */
/* { dg-regexp "    inlined from 'main' at .+/diagnostic-test-inlining-2.c:25:3:" "" } */
/* { dg-warning "28: message" "" { target c } 7 } */
/* { dg-begin-multiline-output "" }
 #define INNER_WARNING(MSG) __emit_warning (MSG)
                            ^~~~~~~~~~~~~~~~~~~~
   { dg-end-multiline-output "" } */
/* { dg-message "28: in expansion of macro 'INNER_WARNING'" "" { target c } 9 } */
/* { dg-begin-multiline-output "" }
 #define OUTER_WARNING(MSG) INNER_WARNING (MSG)
                            ^~~~~~~~~~~~~
   { dg-end-multiline-output "" } */
/* { dg-message "3: in expansion of macro 'OUTER_WARNING'" "" { target c } 14 } */
/* { dg-begin-multiline-output "" }
   OUTER_WARNING ("message");
   ^~~~~~~~~~~~~
   { dg-end-multiline-output "" } */
